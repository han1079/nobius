...
void Orchestrator::shutdown() {
    m_running = false;
    /*Shutoff sequence*/
    while (!m_session_stack.empty()) {
        pop_session();
    }
}

void Orchestrator::push_session(std::unique_ptr<Session> new_session) {
    m_session_stack.push_back(std::move(new_session));
}
...
void Orchestrator::process_session_request() {
    for (const auto& session_name : m_requested_session_push) {
        ...
        // For example:
        // auto new_session = std::make_unique<YourSessionDerivedClass>(session_name, m_entity_manager, m_renderer, m_aggregate_manager, m_input_system);
        // push_session(std::move(new_session));
    }
